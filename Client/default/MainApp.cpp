...
{
	CDataManager::Get_Instance()->Initialization();
...
HRESULT CMainApp::Ready_Component()
{
	...
	/* For.Prototype_Component_Shader_UI */
	if (FAILED(m_pGameInstance->Add_Prototype(LEVEL_STATIC, TEXT("Prototype_Component_Shader_Billboard_UI"),
		CShader::Create(m_pGraphic_Device, TEXT("../Bin/ShaderFiles/Shader_Billboard_UI.hlsl")))))
		return E_FAIL;
...
void CMainApp::Free()
{
	...
	CUserDataManager::Get_Instance()->Free();
	CUserDataManager::Get_Instance()->Destroy_Instance();

	CDataManager::Get_Instance()->Free();
